...
def generate_name_to_opcode(yaml_dict):
    with open('generated/opcode_name_map.cpp', 'w') as fout:
        ...
        fout.write(
            "std::string GetNameByOpcode(Opcode opcode)\n"
            "{\n"
            "    switch(opcode)\n"
            "    {\n"
        )
        ...
        for key in yaml_dict:
            fout.write(f"    case {key.upper()}:\n")
            fout.write(f"        return \"{key}\";\n")
...
def main():
    ...
    generate_name_to_opcode(yaml_dict)
